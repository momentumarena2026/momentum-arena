  ...
  async function handleSendOtp(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/auth/send-otp", {
        ...
        body: JSON.stringify({ email: email.trim(), type: "EMAIL" }),
      });
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
      ...
      <p className="text-zinc-500 text-sm mb-4">
        Quick login with your email to complete the booking.
      </p>
      ...
      {step === "email" && (
        <>
          ...
          <button
            ...
            disabled={loading}
            className="w-full flex items-center justify-center gap-2 bg-white hover:bg-gray-100 text-gray-800 font-medium text-sm py-2.5 rounded-lg transition-colors disabled:opacity-50"
          >
      ...
      {step === "otp" && (
        <form onSubmit={handleVerifyOtp} className="space-y-3">
          <p className="text-sm text-zinc-400">
            OTP sent to <span className="text-emerald-400 font-medium">{email}</span>
          </p>
          <input
            type="text"
            value={otp}
            ...
            required
            maxLength={6}
            className="w-full rounded-lg bg-zinc-800 border border-zinc-700 px-3 py-2.5 text-sm text-white placeholder-zinc-500 focus:outline-none focus:border-emerald-600 text-center tracking-[0.5em] text-lg font-mono"
          />
          <button
            type="submit"
