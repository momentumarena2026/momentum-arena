...
interface Props {
  /** GENERAL (default) or RUNNING — stamps creates + scopes the option chips. */
  module?: ExpenseModule;
  /** Hide the Vendor field and submit `vendor: ""` (RUNNING has no vendor). */
  showVendor?: boolean;
  /** Skip the built-in title block — for hosts (bottom sheet) with their own header. */
  showTitle?: boolean;
  /** undefined ⇒ create, present ⇒ edit existing. */
  expenseId?: string;
  /** Called after a successful save or delete — host closes / navigates back. */
  onSaved: () => void;
...
/**
 * Shared create/edit expense form body. Hosted two ways:
 *   - AdminExpenseFormScreen (GENERAL + all edits) — full-screen route.
 *   - The Running Expenses list's "add" bottom sheet — modal create flow.
 *
...
export function ExpenseFormBody({
  ...
  const detail = useQuery({
    ...
    queryFn: () => adminExpensesApi.detail(expenseId!),
    enabled: isEdit,
  });
  ...
  const submit = useMutation({
    mutationFn: () => {
      const body: AdminExpenseInput = {
        ...
        doneBy: doneBy.trim(),
        toName: toName.trim(),
        // Vendor-less flavors (RUNNING) store "" — the column is NOT NULL.
        vendor: showVendor ? vendor.trim() : "",
        spentType: spentType.trim(),
        note: note.trim() || null,
      };
      return isEdit
        ? adminExpensesApi.update(expenseId!, {
            ...body,
    ...
    onSuccess: () => {
      ...
      void qc.invalidateQueries({ queryKey: ["admin-expense", expenseId] });
  ...
  const remove = useMutation({
    mutationFn: () => adminExpensesApi.remove(expenseId!),
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ["admin-expenses"] });
      void qc.invalidateQueries({ queryKey: ["admin-expense-analytics"] });
      onSaved();
  ...
  const canSubmit = useMemo(() => {
    ...
    return (
      ...
      (!showVendor || vendor.trim().length > 0) &&
