/**
 * The Laws of Cricket — app-side mirror of lib/cricket-rules.ts.
 *
 * A copy rather than an import purely because Metro roots at apps/mobile
 * and cannot resolve the web lib. It exists so a casual match can be
 * scored with no signal; the server's copy is still the authority. The
 * parity suite drives BOTH through the same grid of inputs and asserts
 * identical answers, so this cannot quietly drift the way the two full
 * engines did.
 *
