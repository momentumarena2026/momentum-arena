 ...
 * /admin/analytics/sports. Range chips (30d / 90d / All) drive a
 * getKPIStats fetch; "All" sends no from/to so the route defaults to
 * earliest-confirmed-payment → today (lifetime totals).
 *
 * KPIs: total/sports/cafe earnings, bookings, avg per booking, active
 * customers, cancellation rate. One simple revenue split bar
 * (sports vs cafe). No chart lib — bars are plain Views, matching
 * AdminExpenseAnalyticsScreen.
 */
export function AdminSportsAnalyticsScreen() {
  ...
  const query = useQuery({
    queryKey: ["admin-sports-analytics", from ?? "ALL", to ?? "ALL"],
    queryFn: () =>
      adminAnalyticsApi.sports({ from: from ?? undefined, to: to ?? undefined }),
...
function Body({ data }: { data: SportsAnalyticsResponse }) {
  const k = data.kpi;
  const splitMax = Math.max(k.sportsRevenue, k.cafeRevenue, 1);
  ...
  return (
    <>
      ...
      <Section title="REVENUE SPLIT">
        <BarRow
          label="Sports"
          amount={k.sportsRevenue}
          subtext={`${k.totalBookings.toLocaleString("en-IN")} bookings`}
          max={splitMax}
          color={colors.yellow400}
        />
        <BarRow
          label="Cafe"
          amount={k.cafeRevenue}
          subtext={`${k.totalOrders.toLocaleString("en-IN")} orders`}
          max={splitMax}
