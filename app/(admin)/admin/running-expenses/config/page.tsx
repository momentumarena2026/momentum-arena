...
import { ExpenseConfigClient } from "../../expenses/config/expense-config-client";

export default async function RunningExpenseConfigPage() {
  const options = await listExpenseOptions("RUNNING");

  // Group by field for the client component so it can render one tab
  // per dropdown without re-fetching. RUNNING options start empty by
  // design — the admin builds their own set here.
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-zinc-400">
          ...
          running expense field. Historical rows keep their original labels
          even if you rename or disable an option here.
        </p>
      </div>
