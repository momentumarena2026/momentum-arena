    ...
    lanes: [
      ...
      },
      {
        ...
        steps: [
          ...
          { title: "Package, sign, upload + auto-canary", desc: "Builds the production release and immediately publishes it to a small slice of users (~20%). Only that canary gets it; everyone else keeps their current app.", tags: [{ t: "auto", engine: "GitHub Actions → Vercel Blob" }] },
          { title: "Promote to 100%", desc: "Once the canary looks healthy, Admin bumps the rollout to everyone on the production row of OTA Updates. (Rollback also lives here if it looks bad.)", tags: [{ t: "manual", who: "Admin" }] },
          { title: "Phone fetches it", desc: "Each user’s app downloads it on next open — the canary slice first, the rest after promotion.", tags: [{ t: "auto", engine: "In the app · Expo" }] },
