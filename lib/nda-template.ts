...
// prose here changes every future PDF generated from /admin/nda. The
// placeholders (employee name, Aadhaar, address, etc.) are filled from the
// details entered in the admin HR tool at generation time.
//
...
export type NdaFields = {
  ...
  phone: string;
  email: string;
  aadhaar: string; // full, formatted "1234 5678 9012" — for the PDF ONLY; never stored
  address: string;
...
export type NdaDocument = {
  title: string;
  intro: string[]; // paragraphs before the numbered clauses
  clauses: { heading: string; body: string }[];
  closing: string[]; // paragraphs after the clauses, before signatures
  signature: string[]; // lines rendered in the signature block ("" = blank spacer)
};

export function buildNda(f: NdaFields): NdaDocument {
  return {
    title: "NON-DISCLOSURE AND CONFIDENTIALITY AGREEMENT",
    intro: [
      `This Non-Disclosure and Confidentiality Agreement ("Agreement") is made on ${f.date} at Mathura, Uttar Pradesh, by and between:`,
      `Sportive Ventures, operating under the brand "Momentum Arena", having its principal place of business at Khasra no. 293/5, Mouja Ganeshra, Radhapuram Road, Mathura, Uttar Pradesh 281004 (the "Company"); and`,
      `${f.name}, holding Aadhaar No. ${f.aadhaar}, residing at ${f.address}, contactable at ${f.phone} and ${f.email} (the "Employee").`,
    ],
    clauses: [
      {
        heading: "1. Purpose",
    ...
    closing: [
      "The Employee confirms that they have read and understood this Agreement and sign it voluntarily. IN WITNESS WHEREOF, the parties have executed this Agreement on the date first written above.",
    ],
    signature: [
      "For the Company (Sportive Ventures):",
